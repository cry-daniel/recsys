...
- Aggressive or semantically wrong reuse is the negative control: `window_topk_same_id:action` reaches 3.61% reuse but has max AUC drop 0.000000.
- Constrained same-action reuse is the useful regime: `window_topk_same_id:action` reaches 3.61% reuse with mean AUC diff +0.000107.
